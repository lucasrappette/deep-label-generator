...
{
    public class LabelServiceLabelNumbers : ILabelServiceLabelNumbers
    {
            ...
            try
            {
                ...
                foreach (var order in orderList)
                {
                    try
                    {
                        ...
                        GenerateBmpStopNumbers(stopNumberText, count);
                ...
                string[] pdfsArray = new string[sheetCount];
                for (int pdfs = 0; pdfs < sheetCount; pdfs++)
                {
                ...
                //(
                //    pdfsArray,
                //    Path.Combine(_pathBase, "Downloads" + "\\Labels.png")
        ...
        private static List<Order>? CreateOrderList(string csvPath, int firstField, List<Order>? orderList)
        {
            ...
            try
            {
                using var csvParser = new TextFieldParser
                (
                    csvPath
                );
                ...
                while (!csvParser.EndOfData)
                {
                    try
                    {
                        ...
                        var orderNumber = fields?[firstField];
                        var orderDestination = fields?[firstField + 1];
                        var order = new Order()
                        {
        ...
        {
            var qrCodeZen = Zen.Barcode.BarcodeDrawFactory.CodeQr;
        ...
        private static void GenerateBmpStopNumbers(string? stopNumberText, int count)
        {
            var pathBase = AppDomain.CurrentDomain.BaseDirectory;
            ...
            for (var i = 1; i < 16; i++)
            {
                ...
                font.Dispose();
                graphics.Dispose();
                bmpStopNumber.Save(Path.Combine(pathBase, "Process" + "\\StopNumber" + $"{i}" + ".png"));
